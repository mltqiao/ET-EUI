...
namespace ET
{
    ...
    public static class LoginHelper
    {
        public static async ETTask<int> Login(Scene zoneScene, string address, string account, string password)
        {
            ...
            string realmAdress = httpGetRealmResponse.Realms[modCount];
            Log.Debug($"“登录测试 {account} {password} Realm:{realmAdress}");
            ...
            {
                Account = account, Password = password, LoginWay = (int)LonginWayType.Normal
            });
        ...
        {
            R2C_GetServerList r2CGetServerList = (R2C_GetServerList)await zoneScene.GetComponent<SessionComponent>().Session.Call(new C2R_GetServerList() { });
        ...
        public static async ETTask<int> LoginZone(Scene zoneScene, int zone)
        {
            ...
            Log.Debug($"登陆测试 Gate:{r2CLoginZone.GateAddress} Key:{r2CLoginZone.GateKey}");
            ...
            G2C_Login2Gate g2C_Login2Gate = (G2C_Login2Gate) await gateSession.Call(new C2G_Login2Gate() { GateKey = r2CLoginZone.GateKey });
            if (g2C_Login2Gate.Error != ErrorCode.ERR_Success)
            {
                Log.Error($"登录测试 G2C_Login2Gate Error {g2C_Login2Gate.Error}");
                return g2C_Login2Gate.Error;
            }
        ...
        public static async ETTask<int> GetRoleInfos(Scene zoneScene)
        {
            ...
            {
                Log.Error($"登录测试 C2G_GetRoles Error {g2CGetRoles.Error}");
                return g2CGetRoles.Error;
            }
            
            zoneScene.GetComponent<RoleInfosComponent>().ClearRoleInfo();

            foreach (var gateRoleInfo in g2CGetRoles.Roles)
            {
        ...
        {
            G2C_CreateRole g2CCreateRole = (G2C_CreateRole)await zoneScene.GetComponent<SessionComponent>().Session.Call(new C2G_CreateRole() { Name = name });

            if (g2CCreateRole.Error != ErrorCode.ERR_Success)
            {
                Log.Error($"登录测试 G2C_CreateRole Error Role:{g2CCreateRole.Role}");
        ...
        {
            G2C_DeleteRole g2CDeleteRole = (G2C_DeleteRole)await zoneScene.GetComponent<SessionComponent>().Session.Call(new C2G_DeleteRole() { RoleId = roleId});

            if (g2CDeleteRole.Error != ErrorCode.ERR_Success)
            {
                Log.Error($"登录测试 G2C_DeleteRole Error Role:{g2CDeleteRole.Error}");
        ...
        public static async ETTask<int> EnterMap(Scene zoneScene)
        {
            ...
            G2C_Enter2Map g2CEnter2Map = (G2C_Enter2Map)await zoneScene.GetComponent<SessionComponent>().Session.Call(new C2G_Enter2Map() { UnitId = zoneScene.GetComponent<RoleInfosComponent>().CurrentUnitId});

            if (g2CEnter2Map.Error != ErrorCode.ERR_Success)
            {
                Log.Error($"登录测试 G2C_Enter2Map Error Role:{g2CEnter2Map.Error}");
        ...
        {
            G2C_CancelQueue g2CCancelQueue = (G2C_CancelQueue)await zoneScene.GetComponent<SessionComponent>().Session
                    .Call(new C2G_CancelQueue() { UnitId = zoneScene.GetComponent<RoleInfosComponent>().CurrentUnitId });
